...
- Status: Accepted
- Date: 2026-07-29
- Updated: 2026-07-31 (説明を簡潔化し、費用リスクへの対策を追記)

## Context

CloudFrontから2つのLambda Function URL(api-fn, chat-fn)へ`/api/*`をルーティングする。Function URLはインターネットに公開されるHTTPSエンドポイントである為、CloudFrontを経由しない直接アクセスをどう扱うかを決める必要がある。

CloudFrontにはOrigin Access Control (OAC)があり、オリジンへのアクセスを特定のディストリビューションからのみに限定できる。一方、本システムの認証は、SPAがCognitoで取得したアクセストークンを`Authorization: Bearer`ヘッダーで送り、FastAPIがJWTを検証する方式である(ADR-0004)。課題は、OACによるオリジン保護とBearerトークン認証が両立するかである。
...
本決定は、Function URLのドメインが第三者に知られないことを前提とする。ドメインは推測困難な文字列であるが、シークレットとして管理される値ではない。露出を減らす対策と、前提が破れた場合に備える対策を次の通り行う。

露出を減らす

- FastAPIの末尾スラッシュ自動リダイレクトを無効化する(`redirect_slashes=False`)。307レスポンスのLocationヘッダーからFunction URLのドメインが判明するため
- Function URLをCfnOutputへ出力しない。CI/CDのログやスタックの出力から読み取られる経路を減らす

前提が破れた場合に備える

- api-fnとchat-fnに予約同時実行数を設定する。費用の上限にはならないが、両Functionの同時実行数が頭打ちになるため、アカウントの同時実行枠を他のFunctionへ残せる
- Lambdaの`Invocations`メトリクスにCloudWatchアラームを設定し、AWS Budgetsの閾値通知を併用する。アラームは数分で発報するのに対しBudgetsは請求データの反映に半日程度かかるため、検知はアラーム、最終防波堤はBudgetsという役割分担とする
- ドメインの漏洩や攻撃を検知した場合は、Function URLを削除して再作成する。`url-id`が変わるため攻撃は停止する。作成に費用はかからないが、EdgeStackのオリジン設定を更新して再デプロイするまでAPIは停止する。手順は運用ドキュメントに記載する
...
全APIルートがJWT検証を要求する為、認証を突破されるわけではない。残るのは費用のリスクである。Lambdaの予約同時実行数は同時実行数の上限であってリクエストレートの上限ではないため、費用の天井としては機能しない。ただし他のFunctionへの波及を抑える手段としては有効であり、上記の対策に含めている。
...
CloudFrontがオリジンリクエストへ固定の秘密ヘッダーを付与し、バックエンドが検証する方式である。SigV4署名を伴わない為、`Authorization`ヘッダーと競合しない。
...
- SSEは中間層が増えるほどバッファリングやタイムアウトで壊れやすい。CloudFrontだけでもタイムアウト、KeepAlive、圧縮の3項目を調整しており(architecture.md 9.2)、もう1層挟む複雑さに見合う利点が現時点ではない

加えて、SSEを配信するchat-fnへ適用するにはレスポンスストリーミングが必要となるが、対応するREST APIはCDKのL2コンストラクトが未対応であり、本構成での動作を検証できていない。

REST APIが2025年11月にレスポンスストリーミングへ対応したことで、技術的な障壁は下がりつつある。CDKの対応が進んだ時点で再検討する。
