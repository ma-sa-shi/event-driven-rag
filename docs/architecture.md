...
グラフを実行する前に、当日の利用回数を1回消費する。上限に達している場合は429を返し、Bedrockを呼ばない。上限は1ユーザーあたり1日20回で、`CHAT_DAILY_QUOTA`で変更する。
...
チャットの利用回数は`SK = QUOTA#<JSTの日付>`のアイテムで数える。日付をキーに含めるため、日付が変わると別のアイテムになり、残数を戻す処理を持たない。過去分は`expiresAt`のTTLで自動的に削除する。TTLを設定した属性を持つエンティティはQuotaだけである。
...
Bedrockの推論は上記とは別に従量課金となる。Self-RAGは1チャットで最大8回のLLM呼び出しが発生するため、単価の低いモデルを選んでコストを抑える。実測では1チャット約$0.006で、内訳はLLMが約$0.0038、Rerankが$0.0020である。

推論費用には利用量に応じた上限がないため、1ユーザーあたり1日20回のチャットに制限する。これにより月額費用の上限が確定する。消費の記録でDynamoDBの書き込みがチャット1件につき1回増えるが、上記のチャット1件あたりの単価に対しては無視できる。
...
- [ADR-0017: チャットの利用量を回数制のチケットで制限する](./adr/0017-chat-daily-quota.md)
